...
"""
    ...
    st.markdown('<div class="page-sub">UPI Transaction Forecasting Dashboard — Minor Project</div>',
                unsafe_allow_html=True)
    ...
    with col1:
        ...
        <div style='background:white;border-radius:14px;padding:22px;
            ...
            <div style='font-family:Syne,sans-serif;font-size:18px;
                        font-weight:800;color:#0A1628;margin-bottom:12px;'>
                UPI Live Forecasting Dashboard
            </div>
            A comprehensive time series forecasting and live monitoring system
            for India's UPI transaction infrastructure. Built as a minor project,
            designed as an internal monitoring tool for NPCI.<br><br>

            <strong>Data:</strong> May 2021 – March 2026 (1,796 daily rows)<br>
            <strong>Source:</strong> BHIM / NPCI official statistical releases<br>
            <strong>Targets:</strong> Volume (Mn. transactions/day) and
                                      Value (₹ Crores/day)<br>
            <strong>Models:</strong> 21 models across 6 families<br>
            <strong>Best Model:</strong> Seasonal Naive (lag=7) — MAPE 3.80%<br>
            <strong>Alerts:</strong> Forward-looking forecast alerts +
                                     anomaly detection<br>
            <strong>AI Reports:</strong> News-aware Ollama LLM integration<br>
            <strong>GitHub:</strong>
            <a href="https://github.com/ekshu007/upi-timeseries" target="_blank">
                github.com/ekshu007/upi-timeseries
            </a>
        </div>
        """, unsafe_allow_html=True)

        # Models used
        st.markdown('<div class="section-hdr">🤖 Models Used</div>',
                    unsafe_allow_html=True)
        model_families = {
            "📏 Baselines (6)": "Historical Mean, Median, Rolling Mean (30d), "
                                "Drift, Seasonal Naive lag=7, Seasonal Naive lag=365",
            "📈 Exp. Smoothing (6)": "SES, Holt Linear, Holt Damped, "
                                     "Holt-Winters Additive, "
                                     "Holt-Winters Multiplicative, ETS Auto",
            "🌊 Multi-Seasonal (3)": "MSTL, BATS, TBATS",
            "📊 ARIMA Family (3)": "ARIMA, SARIMA, SARIMAX "
                                   "(with holiday exogenous features)",
            "🔮 Prophet (1)": "Facebook Prophet with 22 Indian holidays "
                              "+ extra regressors",
            "🧠 Deep Learning (2)": "Bidirectional LSTM — "
        ...
        for family, models in model_families.items():
            ...
            <div style='background:#F8FAFC;border-radius:8px;
                ...
                <span style='font-size:12px;color:#64748B;'>{models}</span>
            </div>""", unsafe_allow_html=True)
    ...
    with col2:
        ...
        <div style='background:white;border-radius:14px;padding:18px;
            ...
            <div style='font-family:Syne,sans-serif;font-size:15px;
                        font-weight:700;color:#0A1628;margin-bottom:12px;'>
                📊 Dataset Stats
            </div>
        """, unsafe_allow_html=True)
        ...
        stats = {
            ...
            "Frequency":    "Daily",
            "Total Rows":   f"{len(df):,}",
            "Train Set":    f"{len(df[df['Date'] <= TRAIN_END]):,} days",
            "Test Set":     f"{len(df[df['Date'] >= TEST_START]):,} days",
            "Features":     "17 columns",
            "Exog Features":"9 holiday features",
        }
        for k, v in stats.items():
            ...
            <div style='display:flex;justify-content:space-between;
                ...
                <strong style='color:#0A1628;'>{v}</strong>
            </div>""", unsafe_allow_html=True)
        ...
        <div style='background:white;border-radius:14px;padding:18px;
                    ...
                    margin-bottom:16px;'>
            <div style='font-family:Syne,sans-serif;font-size:15px;
                        font-weight:700;color:#0A1628;margin-bottom:10px;'>
                ▶️ How to Run
            </div>
            <code style='font-size:12px;'>
                pip install -r requirements.txt<br><br>
                streamlit run app.py<br><br>
                # For monthly update:<br>
                python monthly_updater.py<br><br>
                # For AI reports:<br>
                ollama serve<br>
        ...
        <div style='background:white;border-radius:14px;padding:18px;
            ...
            <div style='font-size:13px;color:#475569;line-height:1.8;'>
                <strong style='color:#1A6FBF;'>MAE</strong>
                — Mean Absolute Error<br>
                <strong style='color:#1A6FBF;'>RMSE</strong>
                — Root Mean Square Error<br>
                <strong style='color:#1A6FBF;'>MAPE</strong>
                — Mean Absolute % Error<br><br>
                Test set: Oct–Dec 2025 (92 days)
            </div>
        </div>
        """, unsafe_allow_html=True)
    ...
    <div style='text-align:center;color:#94A3B8;font-size:12px;
        ...
        UPI Transaction Forecasting Dashboard &nbsp;·&nbsp;
        Minor Project &nbsp;·&nbsp;
        Built with Streamlit, Plotly & Ollama &nbsp;·&nbsp;
        Data: BHIM / NPCI
    </div>
    """, unsafe_allow_html=True)